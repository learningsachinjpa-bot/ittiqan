...
async def get_audit_logs(user: User = Depends(get_current_user), db: Session = Depends(get_db), limit: int = 50):
    # HIGH-8: cap limit to prevent DB DoS
    limit = max(1, min(limit, 500))
    membership = db.query(OrgMember).filter(OrgMember.user_id == user.id).first()
    if not membership:
        raise HTTPException(status_code=404, detail="No organization found")
    logs = db.query(AuditLog).filter(AuditLog.org_id == membership.org_id).order_by(AuditLog.created_at.desc()).limit(limit).all()
    return [
        {
            "id": l.id, "action": l.action, "resource_type": l.resource_type,
            "resource_id": l.resource_id, "details": l.details,
            "user_id": l.user_id, "ip_address": l.ip_address,
